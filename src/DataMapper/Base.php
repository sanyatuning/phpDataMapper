...
class Base {
    ...
    protected $_queryClass = 'Query';
    protected $_collectionClass = 'Collection';
     ...
     * 	Constructor Method
     */
    public function __construct(AdapterInterface $adapter, $adapterRead = null) {
        $this->_adapter = $adapter;

        // Slave adapter if given (usually for reads)
        if (null !== $adapterRead) {
            if ($adapterRead instanceof Adapter_Interface) {
                $this->_adapterRead = $adapterRead;
            } else {
                throw new InvalidArgumentException("Secondary/Slave adapter must implement 'Adapter_Interface'");
            }
        }
     ...
     */
    public function getRelationsFor(Entity $entity) {
        ...
        if (count($rels) > 0) {
            foreach ($rels as $column => $relation) {
                ...
                // Load relation class
                $relationClass = 'Relation_' . $relation['relation'];
     ...
     */
    public function getEntityRelationWithValues(Entity $entity) {
        $rels = $this->relations();
        if (count($rels) > 0) {
            foreach ($rels as $column => $relation) {
        ...
        foreach ($entity->toArray() as $field => $value) {
            if ($relationColumns && array_key_exists($field, $relationColumns) && (is_array($value) || is_object($value))) {
                foreach ($value as $relatedRow) {
                    ...
                    // Row object
                    if ($relatedRow instanceof Entity) {
